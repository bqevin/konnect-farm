...
<?php 
    ...
    if (isset($_GET['message'])) {
      ...
      if($message == 2){
        $message = "Your product image could not be uploaded at the moment please try again";
      }elseif ($message == 1) {
        $message = $_SESSION['username'].", lets upload an image for your product";
      }
    }
...
<?php
    ...
    if (!empty($_FILES["uploadedimage"]["name"])) {
      ...
      $imagename = date("d-m-y")."-".time()."pic1".$ext;
      ...
      if ($upload_r) {
        $query_upload = "UPDATE products SET pic1 = '$target_path' WHERE id = $id"
        ...
        if ($result) {
          ...
          $_SESSION['product_success'] = 2;
          header("Location:complete_product_profile.php");
        }
...
<?php 
  if (isset($_SESSION['product_success'])) {
    if ($_SESSION['product_success'] == 2) {
    ?>
      <div class="well">
        <p style="color:blue">Please select an image to upload.</p>
      </div>
      <?php
    }
   }
?>
  <form action="complete_product_profile.php" method="POST" enctype="multipart/form-data">
    <input type="file" name="uploadedimage">
    <input type="submit" name="uploadproduct" value="Upload">
  </form>
